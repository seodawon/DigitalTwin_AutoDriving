...
#
# Author: Leon Jung, Gilbert, Ashe Kim, ChanHyeong Lee,  Rujin Kim
...
class DetectTrafficLight(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info('DetectTrafficLight Node Initialized,rujin')

        if self.sub_image_type == 'compressed':
            self.sub_image_original = self.create_subscription(
                CompressedImage, '/camera/image_raw/compressed', self.get_image, 1)
            self.get_logger().info('CompressedImage: /camera/image_raw/compressed')
        else:
            self.sub_image_original = self.create_subscription(
                Image, '/camera/image_raw', self.get_image, 1)
            self.get_logger().info('Raw Image: /camera/image_raw')
    ...
    def find_traffic_light(self):
        ...
        cv2.putText(self.cv_image, 'find_traffic_light', (1,1),cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 255, 255))
